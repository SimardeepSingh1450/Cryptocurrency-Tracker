...
const Search = ({ setCoinID }) => {
  ...
  const coinsData =
    coinSearchData != null
      ? coinSearchData.data != null
        ? coinSearchData.data.coins
        : null
      : null;
  ...
  return (
    <div className="relative xs:w-full lg:w-96">
      <form
        ...
        className="xs:w-full lg:w-96 relative flex items-center xs:ml-0 lg:ml-2"
      >
        <input
          value={input}
          onChange={(e) => {
          ...
          type="text"
          name="search"
          className="w-full rounded bg-gray-600 placeholder:text-white pl-2 py-0.5 required outline-0 border border-transparent focus:border-green-600"
          placeholder="Search"
        />
